...
import com.capgemini.onlinemedicalstoreusingjpahibernate.beans.CartBean;
	...
	public void cartOperation(int userId) {
		...
		while (validation) {
			try {
				...
				operation = ExceptionMethods.numberValidation(operation1);
				validation = false;
		...
		case 2:// To remove product from cart
			...
			while (validation) {
				try {
					...
					cartId = ExceptionMethods.numberValidation(cartId1);
					validation = false;
				} catch (MedicalExceptions e1) {
					e1.getMessage();
				}
